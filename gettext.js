...
app.post('/', upload, async (req, res) => {
    ...
    try {
        ...
        if (divUrl) {
            const imageUrls = await extractImagesFromURL(divUrl); // ดึงภาพจาก divUrl
            for (const imageUrl of imageUrls) {
                const response = await fetch(imageUrl);
                const buffer = await response.buffer();
                const imagePath = path.join(__dirname, `${uuidv4()}.png`);
                fs.writeFileSync(imagePath, buffer);
                extractedImageFiles.push(imagePath); // เก็บภาพใน extractedImageFiles
            }
            ...
            mainDivContent.find('div').each((i, elem) => {
                ...
                if (divText && !Array.from(seenTexts).some(text => text.includes(divText) || divText.includes(text))) {
                    seenTexts.add(divText);
                    children.push(new Paragraph({ text: divText }));
